...
export default function Dashboard() {
    ...
    const [inboundStuffCount, setInboundStuffCount] = useState(0);
    ...
    //     })
    //         .then(res => {
    //             setLendingGrouped(res.data.data);
    //         })
    //         .catch(err => {
    //             if (err.response && err.response.status === 401) {
    //                 navigate('/login?message=' + encodeURIComponent('Anda belum login!'));
    ...
    function fetchInboundStuffCount() {
        axios.get('http://localhost:8000/inbound/dashboardCalculate', {
            headers: {
                "Authorization": 'Bearer ' + localStorage.getItem('access_token'),
            }
        })
            .then(res => {
                setInboundStuffCount(res.data.count);
            })
            .catch(err => {
                ...
                <div className="p-4 w-1/2">
                    <div className="flex rounded-lg h-full dark:bg-gray-800 bg-teal-400 p-8 flex-col">
                        <div className="flex items-center mb-3">
                            ...
                            <h2 className="text-white dark:text-white text-lg font-medium">Total Inbound Stuff Exceeding 10</h2>
                        </div>
                        <div className="flex flex-col justify-between flex-grow">
                            <h1 className="text-white dark:text-white text-lg font-medium">{inboundStuffCount}</h1>
                        </div>
                    </div>
                </div>
            </div>
